...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
    ...
    // start with first item that's unsorted
...
int main()
{
    ...
    // destruction time.
    {
        ...
        vector<Store*> auxStorePointers(auxStores.size());
        for (int i = 0; i < auxStores.size(); i++)
            auxStorePointers.push_back(&auxStores[i]);
        
        // Sort the vector of pointers using the STL sort algorithm
        //        with compareStorePtr as the ordering relationship.
        sort(auxStorePointers.begin(), auxStorePointers.end(), compareStorePtr);
        ...
        for (int k = 0; k < auxStores.size(); k++)
            auxStores[k] = *auxStorePointers[k];
